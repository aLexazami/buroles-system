...
$stmt = $pdo->prepare("SELECT s.*, 
  g.name AS guardian_name, g.relationship, g.contact_number AS guardian_contact, 
  g.email AS guardian_email, g.occupation, g.employer, g.is_emergency_contact, 
  e.school_year_id, e.grade_section_id, e.enrollment_date, e.previous_school, 
  gs.grade_level_id 
  FROM students s
  JOIN guardians g ON g.student_id = s.id
  JOIN enrollments e ON e.student_id = s.id
  JOIN grade_sections gs ON gs.id = e.grade_section_id
  WHERE s.id = ?");
$stmt->execute([$id]);
...
$old = getFlash('form_data') ?? $student;
...
$sections = [];
if (!empty($old['grade_level_id'])) {
  $stmt = $pdo->prepare("SELECT id, section_label FROM grade_sections WHERE grade_level_id = ?");
  $stmt->execute([$old['grade_level_id']]);
  $sections = $stmt->fetchAll(PDO::FETCH_ASSOC);
}

renderHead('Edit Student');
?>
  ...
  <main class="grid grid-cols-1 md:grid-cols-[auto_1fr]">
    ...
    <section class="p-4 sm:p-6 md:p-8">
      <div class="bg-emerald-300 flex justify-center items-center gap-2 p-2 mb-5 rounded">
        ...
        <h1 class="font-bold text-lg md:text-xl">Edit Student</h1>
      </div>

      <form action="/controllers/admin/update-student.php?id=<?= $id ?>" method="POST" enctype="multipart/form-data" class="bg-white rounded-xl shadow-md p-6 space-y-8">
        <!-- 🖼️ Profile Photo Upload -->
        <div class="flex justify-center">
          <div class="relative group">
            <img id="photoPreview" src="<?= $student['photo_path'] ?: '/assets/img/default-avatar.png' ?>" alt="Student Photo"
        ...
        <div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <!-- LRN -->
            <div class="relative">
              <input type="text" name="lrn" id="lrn" maxlength="12" pattern="\d{12}" inputmode="numeric"
        ...
        <div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <label class="flex items-center gap-2 mt-2">
              <input type="checkbox" name="is_emergency_contact" value="1" class="accent-emerald-600"
                <?= !empty($old['is_emergency_contact']) ? 'checked' : '' ?>>
        ...
        <div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div class="relative">
              ...
              <label
                class="absolute left-4 top-2 text-sm text-gray-500 transition-all peer-placeholder-shown:top-4 peer-placeholder-shown:text-base peer-placeholder-shown:text-gray-400">
                School Year
              </label>
            </div>
